...
const char* ccode_opConv(struct token *tok, struct pnode *leftSupportNode) {
  ...
  case LEX_AND:
    ...
    break;
  case LEX_APOS: {
    ret = (pnode_evalType(pool, leftSupportNode, nullptr)->kind == TYPE_PTR) ? "->" : ".";
    break;
  }
  case LEX_MOD:
    ret = "%";
...
char* ccode_csym(struct type *type, const char *name, bool pedantic, bool toplevel) {
  ...
  case TYPE_STRUCT: {
    ...
    for (size_t i = 0; i < len; ++i) {
      pair = static_cast<spair*>(*list_get(stype->symbols, i));
...
void ccode_printDefaultHeaders(FILE *out) {
  fputs("/* \n"
        " * Generated by the First Step Fork experimental compiler.\n"
  ...
  fputs("#include <stdbool.h>\n"
        ...
        "typedef void* data;\n\n"
        "extern uintptr __forkpow(uintptr, uintptr);\n\n", out);
}
...
bool ccode_imported(char *name) {
  ...
  for (size_t i = 0; i < len; ++i) {
    elem = reinterpret_cast<const char*>(*array_get(imported, i));
    if (!strcmp(elem, name)) {
      return true;
    }
  }
